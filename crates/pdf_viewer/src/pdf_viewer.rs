...
impl PdfView {
    ...
    fn handle_scroll_wheel(
        ...
        match self.view_mode {
            ...
            ViewMode::SinglePage => {
                ...
                if delta > 0.0 && self.current_page < page_count.saturating_sub(1) {
                    self.current_page += 1;
                    cx.notify();
                } else if delta < 0.0 && self.current_page > 0 {
                    self.current_page -= 1;
                    cx.notify();
                }
            }
    ...
    fn render_continuous_scroll(&self, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut spacer_above: f32 = 0.0;
        for page_index in 0..visible_range.start {
            let (_, page_height) = self.page_pixel_size(page_index).unwrap_or((600.0, 800.0));
            spacer_above += page_height + PAGE_GAP;
        }

        let mut pages_container = v_flex().w_full().items_center();

        if spacer_above > scroll_offset {
            pages_container =
                pages_container.child(div().w_full().h(px(spacer_above - scroll_offset)));
        }
        ...
        let mut spacer_below: f32 = total_height;
        for page_index in 0..visible_range.end {
            let (_, page_height) = self.page_pixel_size(page_index).unwrap_or((600.0, 800.0));
            spacer_below -= page_height + PAGE_GAP;
        }
        if spacer_below > 0.0 {
